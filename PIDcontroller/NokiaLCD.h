/*****************************************
Library for interacting with A LCD screen similar 
to the screens found on a Nokia6100 LCD as available
in a Arduino Shield from Sprakfun

(http://www.sparkfun.com/products/9363)

This library is an adapation of a library coded by
Peter Davenport who retains all copyright
...
{
public:
	static NokiaLCD* get_Instance();
	
	void Init(void);
	void Clear(color_t color);
	void Contrast(char setting);

	void SetPixel(color_t color, unsigned char xP, unsigned char yP);
	
	void DrawLine(int xP0, int yP0, int xP1, int yP1, const color_t color);
	void DrawRect(int xP0, int yP0, int xP1, int yP1, const bool fill, const color_t color);
	void DrawCircle (int xPCenter, int yPCenter, int radius, int color, int circleType);
	
	void WriteChar(char c, int xP, int yP, const color_t fColor, const color_t bColor);
	void WriteStr(const char *pString, int xP, int yP, const color_t fColor, const color_t bColor);
	void WriteStrF(const prog_char *ifsh PROGMEM, int xP, int yP, const color_t fColor, const color_t bColor);

protected:
	static NokiaLCD* _instance;
	NokiaLCD(void);
...
// NOKIA TESTER.h
// *************************************************************************************
void delay_us(int yP);
void delay_ms(int yP);
void ioinit(void);
void reset(void);
...
//*		LCD_SCK	Serial Clock	Digital Pin 13
//************************************************************************
...
#define	SCK_PIN		13
	...
	//*	Arduino Mega ports
	#define	LCD_PORT_CS		PORTH, 5
	#define	LCD_PORT_SCK	PORTB, 6
	#define	LCD_PORT_RES	PORTH, 5
	#define	LCD_PORT_DIO	PORTB, 7
#else
	...
	#define CS_CLEAR() bitClear(LCD_PORT_CS,CS)
#endif
...
// Must be even
#define FONT_WIDTH 8
//Number of pixels tall for each character
#define FONT_HEIGHT 16
//Number of bytes for each character
#define FONT_STRIDE	16
//First Ascii char defined in the font
#define FONT_ASCII_OFFSET 0x1F
